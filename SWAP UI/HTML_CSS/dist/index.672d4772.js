    ...
    if (window.ethereum) {
        ...
        try {
            ...
            ethereum.on("chainChanged", (chainId)=>{
                ...
                } else console.error("Failed to switch to Arbitrum network. Please switch manually.");
            });
            ...
            } else console.error("No accounts found. Please check your MetaMask setup.");
            ethereum.on("accountsChanged", (accounts)=>{
                if (accounts.length > 0) {
                    console.log("Connected to MetaMask");
                    connectButton.innerText = "Connected to MetaMask! \u2705"; // Update button text
                } else console.log("Not connected to MetaMask");
            });
        } catch (error) {
            console.error("Error connecting to Arbitrum network:", error);
        }
    } else alert("Metamask not detected. Please install Metamask to use this application.");
}
...
async function sendToken() {
    ...
    try {
        ...
        await contract.methods.transfer(recipientAddress, amountInSmallestUnit.toString()).send(transactionOptions).on("transactionHash", function(hash) {
...
async function initKeplr() {
    ...
    if (window.getOfflineSigner && window.keplr) try {
...
async function connectKeplr() {
    ...
    if (isKeplrInitialized) {
        ...
        try {
            ...
            } else console.log("No accounts found in Keplr wallet.");
        } catch (error) {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (connectButton1) connectButton1.addEventListener("click", connectMetaMask);
});
